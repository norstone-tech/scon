...
scon.decodeValue = function(buf,type,_offset){
	...
	if (type>=scon.int8 && type<=scon.int48){
		...
		switch( type ){
			...
			case scon.float32:
				result = buf.readFloatE(_offset); _offset += 4;
				break;
...
scon.decode = function ( string, verify, offset ){
	...
	var magicLen = scon.magicNumber.length;
	
	// confirm we are dealing with an scon file
	if ( verify && buf.toString( "binary", 0,scon.magicNumber.length ) != scon.magicNumber ){
		if (buf.toString( "binary", 0,scon.magicNumber.length ) != scon.magicNumberOld){ //Not a "new" scon file, probably an old one.
			magicLen = scon.magicNumberOld.length;
		}else{
	...
	while ( _offset < buf.length && _offset >= 0){
		...
		if (type==scon.endBlock){
			...
			var name = buf.toString("binary", _offset, _offset + nameLength); _offset += nameLength;
