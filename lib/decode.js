...
class InternalSconDecodeData {
	...
	// So the reason why I'm using callbacks is because decoding must work in a both syncronous and asyncronous enviroment.
	// Callbacks are the fastest way (I'm aware of) to achieve this.
	callbackRead(len, cb){
		let endOffset = this.offset + len;
		if (endOffset <= this.buff.length){
			let subBuffer = this.buff.subarray(this.offset, endOffset);
			this.offset = endOffset;
			cb(null, subBuffer); // TODO: Take this oppertunity to reduce the callstack by calling this function in a while loop in scon.decode if decoding syncronously
		}else{
			(async () => {
				try{
					let buff;
	...
	switch(dataType){
		...
		case scon.float32:
			decodeObj.callbackRead(4, (err, floatSlice) => {
				try{
					if (err) throw err;
					let floatBuffer = BufferLib.allocUnsafeSlow(4);
		...
		case scon.float64:
			decodeObj.callbackRead(8, (err, floatSlice) => {
				try{
					if (err) throw err;
					let floatBuffer = BufferLib.allocUnsafeSlow(8);
					floatBuffer.set(floatSlice);
					floatBuffer.reverse();
		...
		default:
			...
			if (defaultValue === undefined){
				throw new scon.Error("unknownObjectType", dataType);
			}
						...
						switch(defaultValue){
							...
							case scon.utf8string0:
								decodeObj.callbackRead(int, (err, stringBuffer) => {
									try{
										if (err) throw err;
										applyDecodedValue(decodeObj, key, array, BufferLib.bufferToString(stringBuffer), cb);
									}catch(ex){
										cb(ex);
									}
								});
							...
							case scon.string0:
								decodeObj.callbackRead(int, (err, bufferSlice) => {
									try{
										if (err) throw err;
										let newBuffer = BufferLib.allocUnsafeSlow(int);
							...
							break;
							case scon.valuePointer0:
								let value = decodeObj.pointedData[int];
								if (value === undefined){
...
const decodeObject = function(decodeObj, cb){
	decodeObj.callbackRead(1, (err, dataType) => {
		try{
			...
			let shortKey = Boolean(dataType & 128);
			if (shortKey){
				dataType ^= 128;
			}
