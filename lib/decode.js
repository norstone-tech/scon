...
class SconDecoder {
	/**
	 * @param {SconDecodeOptions} options
	 */
	constructor(options = {}){
		/**
		 * @type {SconDecodeOptions}
	...
	_parseVarInt(isNegative = false, extendedType = 0){
		...
		if(extendedType >= EXTENDED_TYPES.INT_VAR.UINT64 && extendedType <= EXTENDED_TYPES.INT_VAR.SINT256){
			return BigInt(result);
		}else if(extendedType >= EXTENDED_TYPES.INT_VAR.UINT8 && extendedType <= EXTENDED_TYPES.INT_VAR.SINT32){
	 ...
	 */
	_parseNullTermString(/* istanbul ignore next */ isUtf8 = false, /* istanbul ignore next */ copy = false){
		const start = this._i;
	 ...
	 * @returns {string | Buffer}
	 */
	_parseLengthPrefixedString(/* istanbul ignore next */ isUtf8 = false, /* istanbul ignore next */ copy = false){
	 ...
	 */
	_decode(){
		/* TODO: This was a lot more performant before the decision was made to allow any value to be the root value
		   Try to compare with commit e0b3e21325da0f2b421cfbb6e87321f75bcee8c4 which consistently parsed faster than
		   JSON */
		let parsingArray = (this._curObj instanceof Array) || (this._curObj instanceof Set);
		while(true){
			...
			if(baseType === BASE_TYPES.EOF){
				...
				// We've ended the root object
				if(!this._nestObj.length && this.result != null){
					break;
				}
				parsingArray = (this._curObj instanceof Array) || (this._curObj instanceof Set);
			...
			}else if(this._nestObj.length && !parsingArray){
				if(keyIsReference){
			...
			}else if(this._nestObj.length){
				this._addValue(keyValue, value);
			}else{
				this.result = value;
				if(valueIsReference || baseType != BASE_TYPES.OBJECT){
					break;
				}
			}
			if(baseType == BASE_TYPES.OBJECT && !valueIsReference){
				this._nestObj.push(this._curObj);
				// this._nestExtType.push(this._curExtType);
				this._curObj = value;
				parsingArray = typeVariant;
				// this._curExtType = extendedType;
				if(typeVariant && extendedType == EXTENDED_TYPES.OBJECT.ANY_KEY_MAP){
	...
	_checkMagicNumber(){
		if(this.options.magicNumber){
			if((this._i + MAGIC_NUMBER.length) > this._buffer.length){
				throw new SconTruncateError();
			}
			if(!this._buffer.slice(this._i, this._i + MAGIC_NUMBER.length).equals(MAGIC_NUMBER)){
	...
	decodePartial(buffer){
		if(this._resumeI < this._buffer.length){
			this._buffer = Buffer.concat([this._buffer.slice(this._resumeI), buffer]);
		}else{
