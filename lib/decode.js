...
class SconDecoder {
	...
	_addValue(key, value){
		if(this._curObj == null && key === undefined){
			this.result = value;
			return;
		}
	...
	_decode(){
		while(true){
			...
			}else if(this._curObj != null && !Array.isArray(this._curObj) && !(this._curObj instanceof Set)){
				if(keyIsReference){
			...
			if(
				...
				if(value instanceof Map && typeVariant && extendedType == EXTENDED_TYPES.OBJECT.ANY_KEY_MAP){
			...
			}else if(keyValue === undefined && this._curObj == null){
				// Only possible if we're parsing a root value which isn't an object
				break;
			}
		}
		this._resumeI = this._i;
