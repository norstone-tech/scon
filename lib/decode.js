...
const typedArrayToUnicodeString = function(ua) {
...
const checkBufferLength = function*(bo,length){
	if (bo.offset + length > bo.buff.length){
		...
		const unreadArr = new Uint8Array(bo.buff.length-bo.offset);
	...
	if (type>=scon.int8 && type<=scon.int64){
		...
		switch( type ){
			...
			case scon.float32:
				...
				const LEFloat = new Uint8Array(4);
			...
			case scon.float64:
				...
				//JS handles floats as LE, but SCON stores them as BE, so we gotta convert!
				const LEFloat = new Uint8Array(8);
				for (let i=0;i<LEFloat.length;i+=1){
					LEFloat[i] = bo.buff[bo.offset+LEFloat.length-i-1];
				}
...
scon.decodeKey = function*(bo,shortType){
	...
	const key = typedArrayToUnicodeString(new Uint8Array(bo.buff.buffer,bo.offset,keylen));
	bo.offset += keylen;
	bo.stack.push(key);
}
