...
const applyDecodedValue = function(decodeObj, key, array, value, cb){
	try{
		if (key === undefined){
			...
			decodeObj.decodeObject[key] = value;
		}
		cb(null);
	}catch(ex){
		cb(ex);
...
const decodeValue = function(decodeObj, dataType, key, array, cb){
	switch(dataType){
		...
		default:
			...
			}else if (dataType <= scon.valuePointer64 && dataType >= scon.valuePointer8){
	...
	decodeObj.callbackRead(1, (err, dataType) => {
		try{
			...
			if (dataType === scon.referencedKey){
				readKey(decodeObj, longKey, (err, key) => {
					if (err){
						cb(err);
					}else{
