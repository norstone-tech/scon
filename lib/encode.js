...
class InternalSconEncodeData {
	...
	dataArrayPush(data){
		const dataArray = this.dataArrays[this.scopeArray[this.scopeArray.length - 1]];
		if(data instanceof Uint8Array){
			this.resultLen += data.length;
		}else{
			const refCount = data.keyRef ? this.keyArrayRefCount : this.dataArrayRefCount;
	...
	newScope(){
		const newScope = this.dataArrays.length;
...
const encodeDouble = function(encodeObj, key, value){
	if(value == Infinity){
		...
		const floatInterface = new Float64Array(1);
...
const uintByteLength = function(n){
	if(n === 0){
		return 0;
...
const encodeInt = function(encodeObj, key, value, datatype){
	...
	if(value === 0){
		...
		if(byteLength != -1){
			...
			buff[offset + i] = value;
			while(--i >= 0 && (mul *= 0x100)){
				if(value < 0 && sub === 0 && buff[offset + i + 1] !== 0) sub = 1;
...
const encodeThing = function(encodeObj, key, value){
	switch(typeof value){
		...
		/* istanbul ignore next */
		default:
			throw new Error("Unknown object type: " + value);
	}
};
...
scon.encode = function(obj, options = {useMagicNumber: true}){
	...
	const useMagicNumber = options.useMagicNumber || options.useMagicNumber == null;
	if(useMagicNumber){
		encodeObj.resultLen += scon.magicNumberArr.length;
	}
	const finalBuffer = BufferLib.newBufferUnsafe(encodeObj.resultLen);
	let offset = 0;
	if(useMagicNumber){
		finalBuffer.set(scon.magicNumberArr, offset);
		offset += scon.magicNumberArr.length;
	}
