...
scon.writeInt = function(key,value,datatype){
	...
	if (value === 0){
		...
		if (byteLength != -1){
			...
			const buff = new Uint8Array(keybuff.length+byteLength);
			buff.set(keybuff);
...
scon.writeKey = function(type,key){
	...
	if (key==null){
		...
		const keyArr = unicodeStringToTypedArray(key);
		if (keyArr.length > 255){
			const fullArr = new Uint8Array(keyArr.length + 3);
			fullArr[0] = type;
			fullArr[1] = keyArr.length >>> 8;
			fullArr[2] = keyArr.length; //Store 16 bit int BE
			fullArr.set(keyArr, 3);
		}else{
			const fullArr = new Uint8Array(keyArr.length + 2);
			fullArr[0] = type | 128;
...
scon.writeValue = function(key,value){
	...
	switch( typeof value ){
		...
		case "string":
			const byteStr = unicodeStringToTypedArray(value);
		...
		case "object":
			...
			if (Array.isArray(value)){
				...
				const obj = scon.encode(value,true);
...
scon.encode = function(object,doNotWriteMagicNumber){
	...
	Object.keys(object).forEach(function(key) {
		...
		const c = new Uint8Array(buff.length + valbuff.length);
		c.set(buff);
		c.set(valbuff, buff.length);
		buff = c;
	});
	
	const c = new Uint8Array(buff.length + 1); // Too bad there's no way to append to a type array
	c.set(buff);
